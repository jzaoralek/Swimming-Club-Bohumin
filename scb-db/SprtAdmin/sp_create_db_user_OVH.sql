...
CREATE DEFINER=`sprt_admin`@`localhost` PROCEDURE `create_db_user`(IN  `dbName_in` VARCHAR(25), IN  `usrName_in` VARCHAR(75), IN  `usrPassword_in` VARCHAR(25))
BEGIN
	...
	SET @SQLStmtUsr = CONCAT('CREATE USER \'', usrName_in, '\'', '@', '\'%\'', ' IDENTIFIED BY \'', usrPassword_in, '\'');
    PREPARE StmtUsr FROM @SQLStmtUsr;
	EXECUTE StmtUsr;
	DEALLOCATE PREPARE StmtUsr;
    
    SET @SQLStmtPriv = CONCAT('GRANT ALL PRIVILEGES ON ', dbName_in, '. * TO \'', usrName_in, '\'', '@', '\'%\'');
