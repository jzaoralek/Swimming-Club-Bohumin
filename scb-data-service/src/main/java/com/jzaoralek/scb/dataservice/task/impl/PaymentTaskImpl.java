...
public class PaymentTaskImpl implements PaymentTask {
	...
	public void updateBankPayments() {
		try {
			...
			mailService.sendMail(DataServiceConstants.PLATBY_EMAIL, PROCESS_PAYMENT_SUCCESS_MAIL_SUBJECT, "Zpracovano " + newPaymentCount + " novych plateb.", null);
		} catch (Exception e) {
			// notification email about processing with error
			String exceptionStr = ExcUtil.traceMessage(e).toString();
			mailService.sendMail(DataServiceConstants.ADMIN_EMAIL, PROCESS_PAYMENT_ERROR_MAIL_SUBJECT, exceptionStr, null);
			mailService.sendMail(DataServiceConstants.PLATBY_EMAIL, PROCESS_PAYMENT_ERROR_MAIL_SUBJECT, exceptionStr, null);
	...
	public void processPaymentPairing() {
		try {
			...
			// notification email about success processing
			mailService.sendMail(DataServiceConstants.PLATBY_EMAIL, PAIRING_PAYMENT_SUCCESS_MAIL_SUBJECT, "Zparovano " + processPaymentCount + " plateb.", null);
		} catch (Exception e) {
			// notification email about processing with error
			String exceptionStr = ExcUtil.traceMessage(e).toString();
			mailService.sendMail(DataServiceConstants.ADMIN_EMAIL, PAIRING_PAYMENT_ERROR_MAIL_SUBJECT, exceptionStr, null);
			mailService.sendMail(DataServiceConstants.PLATBY_EMAIL, PAIRING_PAYMENT_ERROR_MAIL_SUBJECT, exceptionStr, null);
